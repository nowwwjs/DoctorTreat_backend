...
import com.doctorTreat.app.doctorClinic.DoctorClinicChating;

// 운영자 유저에서 서버간의 웹 소켓 url
@ServerEndpoint("/MemberClinicChating")
//	public class Admin {
public class MemberClinicChating {
...
//	}
	...
	public void handleMessage(String message, Session userSession) throws IOException {
		...
		DoctorClinicChating.sendMessage(key, msg);
	}

	// 접속이 끊기면 위 운영자 세션을 null 처리한다.
	@OnClose
	public void handleClose(Session userSession) {
		MemberClinicChating = null;
	}
	...
	private static void send(String message) {
		if (MemberClinicChating != null) {
			try {
				MemberClinicChating.getBasicRemote().sendText(message);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
