...
public class MemberReceiveController implements Execute {
	@Override
...
//		로그인정보 담기
		...
		List<ChatDTO> newMessages = memberClinicDAO.getNewMessages(sessionNumber);
		...
		for (ChatDTO message : newMessages) {
			...
			jsonMessage.put("sender", message.getDoctorName()); // 환자는 의사의 이름을 가져와야 하므로
