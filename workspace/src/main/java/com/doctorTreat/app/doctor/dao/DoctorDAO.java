...
public class DoctorDAO {
	...
	public boolean quitDoctor(String doctorId, String doctorPw) {
	    ...
	    // 의사 정보 삭제 (doctorQuit1)
	    int result1 = sqlSession.delete("doctorMypage.doctorQuit1", doctorDTO);

	    System.out.println("doctorQuit3 result: " + result1);
	    System.out.println("doctorQuit2 result: " + result2);
	    System.out.println("doctorQuit1 result: " + result3);

	    // 모든 삭제 쿼리가 성공적으로 실행되었는지 확인
	    if (result1 == 1 && result2 == 1 && result3 == 1) {
	        sqlSession.commit();  // 성공 시 커밋
	        return true;
	    } else {
	        sqlSession.rollback();  // 실패 시 롤백
	        return false;
