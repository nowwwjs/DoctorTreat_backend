...
//    10) 가입버튼 눌렀을 때 입력 안된 창 있으면 "입력란 확인" alert 창 띄우기
document.querySelector(".signup-btn").addEventListener("click", function(event) {
	...
	if (!allFilled || !allCheckboxesChecked) {
		...
		window.location.href = "<%= request.getContextPath() %>/app/user/memberJoinFinish.jsp"; // 이동
	}
});
...
$(document).ready(function() {
	$("#member-join-CheckId-Btn").on('click', function() {
		console.log("클릭");
		let memberId = $('#memberId').val();

		$.ajax({
			url: "/member/memberLoginOk.me",
			type: "get",
