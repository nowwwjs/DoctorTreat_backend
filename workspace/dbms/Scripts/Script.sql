...
--유저타입(의사,일반)테이블
CREATE TABLE DT_USER_TYPE (
    USER_TYPE_NUMBER NUMBER,
    MEMBER_NUMBER NUMBER,
    DOCTOR_NUMBER NUMBER,
    USER_TYPE_TYPE VARCHAR2(200),
    CONSTRAINT PK_USER_TYPE PRIMARY KEY (USER_TYPE_NUMBER),
    CONSTRAINT FK_MEMBER_NUMBER FOREIGN KEY (MEMBER_NUMBER) REFERENCES DT_MEMBER(MEMBER_NUMBER),
    CONSTRAINT FK_DOCTOR_NUMBER FOREIGN KEY (DOCTOR_NUMBER) REFERENCES DT_DOCTOR(DOCTOR_NUMBER)
);

SELECT * FROM DT_USER_TYPE;

-- DT_USER_TYPE 테이블에 시퀀스 생성
...
CREATE TABLE DT_MEDICAL_INFO (
    ...
    MEDICAL_INFO_SUBJECT VARCHAR2(200) NOT NULL,
